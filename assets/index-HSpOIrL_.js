...
<section class="gifts section">
  ...
  <div class="gifts__cards"></div>
</section>`;const h="/christmas-shop/",b={[h]:p,[h+"gifts"]:_,"/":p,"/gifts":_};document.addEventListener("click",t=>{var r;const e=t.target.closest("a, button, [data-url]");if(!e)return;const s=e.tagName==="A"&&e.href.includes("#"),n=e.closest("footer");if(s||n)return;const o=((r=e.dataset)==null?void 0:r.url)||(e==null?void 0:e.href);o&&(t.preventDefault(),$(o))});const $=t=>{let e=new URL(t,window.location.href).pathname;(window.history.state||{}).path!==e&&(window.history.pushState({path:e},"",e),v(),window.scrollTo({top:0}))},v=()=>{const t=window.location.pathname,e=i=>i.endsWith("/")?i:i+"/",s=e(t),n=e(h),o=s===n||s==="/",r=document.getElementById("main"),a=b[t]||b[s]||p;r.innerHTML=a,O(o?4:null)};window.onpopstate=v;v();const F=document.getElementById("burger__button"),L=document.getElementById("burger__menu");F.addEventListener("click",()=>{E(),M(),S()});L.addEventListener("click",t=>{t.target.classList.contains("burger__menu-link")&&(E(),M(),S())});function E(){L.classList.toggle("burger__menu--active")}function M(){F.classList.toggle("burger__button--active")}function S(){document.body.classList.toggle("no-scroll")}const H={childList:!0,subtree:!0},U=(t,e)=>{const s=document.getElementById("scroll-to-top-button");if(s){const n=()=>{const i=window.scrollY,l=window.innerWidth;i>300&&l<=768?s.style.display="flex":s.style.display="none"},o=()=>{window.scrollTo({top:0,behavior:"smooth"})};s.addEventListener("click",o),document.addEventListener("scroll",n),window.addEventListener("resize",n);const r=()=>{s.removeEventListener("click",o),document.removeEventListener("scroll",n),window.removeEventListener("resize",n)},a=new MutationObserver(()=>{document.body.contains(s)||(r(),a.disconnect())});a.observe(document.body,H)}},J=new MutationObserver(U);J.observe(document.body,H);let m,g,k,d=0,u,c=0;const K=document.querySelector(".slider"),w=document.querySelector(".slider__slides"),X=document.querySelector(".content"),C=document.querySelector(".slider__arrow--left"),x=document.querySelector(".slider__arrow--right");x.addEventListener("click",t=>B(t,"next"));C.addEventListener("click",t=>B(t,"prev"));w.addEventListener("transitionend",()=>{f()});window.addEventListener("load",()=>{const t=new ResizeObserver(e=>{const s=e[0],n=e[1],o=e[2];m!=s.contentRect.width&&(d=0,c=0,W(),f(0)),m=s.contentRect.width;const r=n.contentRect.width,a=o.target.scrollWidth;u=m>768?3:6,k=a-r,g=Math.ceil(k/u)});t.observe(X),t.observe(K),t.observe(w)});function f(t){w.style.transform=`translateX(${t}px)`}function B(t,e){e==="next"&&c<u?(c++,d-=g):e==="prev"&&c>0&&(c--,d+=g),W(),f(d)}function W(){C.classList.toggle("slider__arrow--disabled",c===0),x.classList.toggle("slider__arrow--disabled",c===u)}function I(){const t=new Date,e=t.getFullYear()+1,n=new Date(Date.UTC(e,0,1,0,0,0))-t,o=Math.floor(n/(1e3*60*60*24)),r=Math.floor(n%(1e3*60*60*24)/(1e3*60*60)),a=Math.floor(n%(1e3*60*60)/(1e3*60)),i=Math.floor(n%(1e3*60)/1e3);document.getElementById("days").textContent=o,document.getElementById("hours").textContent=r,document.getElementById("minutes").textContent=a,document.getElementById("seconds").textContent=i}setInterval(I,1e3);I();
//# sourceMappingURL=index-HSpOIrL_.js.map
