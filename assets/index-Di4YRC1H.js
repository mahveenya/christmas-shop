...
<section class="gifts section">
  ...
  <div class="gifts__cards"></div>
</section>`;const N={"/mahveenya-JSFE2024Q4/christmas-shop/":z,"/mahveenya-JSFE2024Q4/christmas-shop/gifts":q};document.addEventListener("click",t=>{var r;const e=t.target.closest("a, button, [data-url]");if(!e)return;const s=e.tagName==="A"&&e.href.includes("#"),a=e.closest("footer");if(s||a)return;const o=((r=e.dataset)==null?void 0:r.url)||(e==null?void 0:e.href);o&&(t.preventDefault(),R(o))});const R=t=>{let e=t;t.includes("http")&&(e=new URL(t).pathname),(window.history.state||{}).path!==e&&(window.history.pushState({path:e},"",e),h(),window.scrollTo({top:0}))},h=()=>{const t=window.location.pathname,e=t==="/mahveenya-JSFE2024Q4/christmas-shop/",s=document.getElementById("main");s.innerHTML=N[t],G(e?4:null)};window.onpopstate=h;h();const f=document.getElementById("burger__button"),_=document.getElementById("burger__menu");f.addEventListener("click",()=>{b(),k(),F()});_.addEventListener("click",t=>{t.target.classList.contains("burger__menu-link")&&(b(),k(),F())});function b(){_.classList.toggle("burger__menu--active")}function k(){f.classList.toggle("burger__button--active")}function F(){document.body.classList.toggle("no-scroll")}const L={childList:!0,subtree:!0},J=(t,e)=>{const s=document.getElementById("scroll-to-top-button");if(s){const a=()=>{const c=window.scrollY,l=window.innerWidth;c>300&&l<=768?s.style.display="flex":s.style.display="none"},o=()=>{window.scrollTo({top:0,behavior:"smooth"})};s.addEventListener("click",o),document.addEventListener("scroll",a),window.addEventListener("resize",a);const r=()=>{s.removeEventListener("click",o),document.removeEventListener("scroll",a),window.removeEventListener("resize",a)},n=new MutationObserver(()=>{document.body.contains(s)||(r(),n.disconnect())});n.observe(document.body,L)}},$=new MutationObserver(J);$.observe(document.body,L);let m,p,w,d=0,u,i=0;const U=document.querySelector(".slider"),g=document.querySelector(".slider__slides"),Q=document.querySelector(".content"),E=document.querySelector(".slider__arrow--left"),S=document.querySelector(".slider__arrow--right");S.addEventListener("click",t=>M(t,"next"));E.addEventListener("click",t=>M(t,"prev"));g.addEventListener("transitionend",()=>{v()});window.addEventListener("load",()=>{const t=new ResizeObserver(e=>{const s=e[0],a=e[1],o=e[2];m!=s.contentRect.width&&(d=0,i=0,H(),v(0)),m=s.contentRect.width;const r=a.contentRect.width,n=o.target.scrollWidth;u=m>768?3:6,w=n-r,p=Math.ceil(w/u)});t.observe(Q),t.observe(U),t.observe(g)});function v(t){g.style.transform=`translateX(${t}px)`}function M(t,e){e==="next"&&i<u?(i++,d-=p):e==="prev"&&i>0&&(i--,d+=p),H(),v(d)}function H(){E.classList.toggle("slider__arrow--disabled",i===0),S.classList.toggle("slider__arrow--disabled",i===u)}function C(){const t=new Date,e=t.getFullYear()+1,a=new Date(Date.UTC(e,0,1,0,0,0))-t,o=Math.floor(a/(1e3*60*60*24)),r=Math.floor(a%(1e3*60*60*24)/(1e3*60*60)),n=Math.floor(a%(1e3*60*60)/(1e3*60)),c=Math.floor(a%(1e3*60)/1e3);document.getElementById("days").textContent=o,document.getElementById("hours").textContent=r,document.getElementById("minutes").textContent=n,document.getElementById("seconds").textContent=c}setInterval(C,1e3);C();
//# sourceMappingURL=index-Di4YRC1H.js.map
