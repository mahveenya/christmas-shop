...
<section class="gifts section">
  ...
  <div class="gifts__cards"></div>
</section>`;const L="/christmas-shop/",x={[L]:k,[L+"gifts"]:S,"/":k,"/gifts":S};document.addEventListener("click",t=>{var r;const e=t.target.closest("a, button, [data-url]");if(!e)return;const s=e.tagName==="A"&&e.href.includes("#"),n=e.closest("footer");if(s||n)return;const o=((r=e.dataset)==null?void 0:r.url)||(e==null?void 0:e.href);o&&(t.preventDefault(),Q(o))});const Q=t=>{let e=new URL(t,window.location.href).pathname;(window.history.state||{}).path!==e&&(window.history.pushState({path:e},"",e),F(),window.scrollTo({top:0}))},F=()=>{const t=window.location.pathname,e=i=>i.endsWith("/")?i:i+"/",s=e(t),n=e(L),o=s===n||s==="/",r=document.getElementById("main"),a=x[t]||x[s]||k;r.innerHTML=a,U(o?4:null)};window.onpopstate=F;F();const I=document.getElementById("burger__button"),T=document.getElementById("burger__menu");I.addEventListener("click",()=>{A(),G(),P()});T.addEventListener("click",t=>{t.target.classList.contains("burger__menu-link")&&(A(),G(),P())});function A(){T.classList.toggle("burger__menu--active")}function G(){I.classList.toggle("burger__button--active")}function P(){document.body.classList.toggle("no-scroll")}const j={childList:!0,subtree:!0},ee=(t,e)=>{const s=document.getElementById("scroll-to-top-button");if(s){const n=()=>{const i=window.scrollY,d=window.innerWidth;i>300&&d<=768?s.style.display="flex":s.style.display="none"},o=()=>{window.scrollTo({top:0,behavior:"smooth"})};s.addEventListener("click",o),document.addEventListener("scroll",n),window.addEventListener("resize",n);const r=()=>{s.removeEventListener("click",o),document.removeEventListener("scroll",n),window.removeEventListener("resize",n)},a=new MutationObserver(()=>{document.body.contains(s)||(r(),a.disconnect())});a.observe(document.body,j)}},te=new MutationObserver(ee);te.observe(document.body,j);let b,E,B,u=0,m,l=0;const p=document.querySelector(".slider"),c=document.querySelector(".slider__slides"),h=document.querySelector(".content"),y=document.querySelector(".slider__arrow--left"),_=document.querySelector(".slider__arrow--right");_&&_.addEventListener("click",t=>z(t,"next"));y&&y.addEventListener("click",t=>z(t,"prev"));c&&c.addEventListener("transitionend",()=>{M()});window.addEventListener("load",()=>{if(!h||!p||!c)return;const t=new ResizeObserver(e=>{const s=new Map(e.map(C=>[C.target,C])),n=s.get(h),o=s.get(p),r=s.get(c),a=n?n.contentRect.width:h.getBoundingClientRect().width,i=o?o.contentRect.width:p.getBoundingClientRect().width,d=r?r.target.scrollWidth:c.scrollWidth;b!=a&&(u=0,l=0,O(),M(0)),b=a,m=b>768?3:6,B=d-i,E=Math.ceil(B/m)});t.observe(h),t.observe(p),t.observe(c)});function M(t=u){c&&(c.style.transform=`translateX(${t}px)`)}function z(t,e){m&&(e==="next"&&l<m?(l++,u-=E):e==="prev"&&l>0&&(l--,u+=E),O(),M(u))}function O(){!y||!_||(y.classList.toggle("slider__arrow--disabled",l===0),_.classList.toggle("slider__arrow--disabled",l===m))}const g=document.getElementById("days"),v=document.getElementById("hours"),f=document.getElementById("minutes"),w=document.getElementById("seconds");function W(){if(!g&&!v&&!f&&!w)return;const t=new Date,e=t.getFullYear()+1,n=new Date(Date.UTC(e,0,1,0,0,0))-t,o=Math.floor(n/(1e3*60*60*24)),r=Math.floor(n%(1e3*60*60*24)/(1e3*60*60)),a=Math.floor(n%(1e3*60*60)/(1e3*60)),i=Math.floor(n%(1e3*60)/1e3);g&&(g.textContent=o),v&&(v.textContent=r),f&&(f.textContent=a),w&&(w.textContent=i)}(g||v||f||w)&&(W(),setInterval(W,1e3));
//# sourceMappingURL=index-CvVLeOKg.js.map
